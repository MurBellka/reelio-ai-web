  ...
  return function authMiddleware(req, _res, next) {
    ...
    verifier
      ...
      .catch((err) => {
        ...
        // Наружу причина не уходит: истёкший, поддельный и отозванный токен
        // для клиента обязаны быть неразличимы. Но в лог код писать НУЖНО —
        // без него ошибка конфигурации (например, нехватка прав у сервисного
        // аккаунта) выглядит точно так же, как поддельный токен.
        console.warn(`[auth] token rejected: ${err?.code || err?.errorInfo?.code || 'unknown'}`);
