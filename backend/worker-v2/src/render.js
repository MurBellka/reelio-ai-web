...
import { normalizeOverlay, overlaysToAss } from './textoverlay.js';
import { buildTimeline } from './timeline.js';
import { buildVerifiedCatalog } from './transitions.js';
...
async function renderInside({ env, storage, reporter, logger, workDir, sourcesDir, outDir }) {
  ...
  // §2.2: каталог переходов — пересечение таблицы контракта с тем, что реально
  // умеет эта сборка FFmpeg. UI получает его же, поэтому недоступный переход
  // выбрать нельзя.
  const verifiedCatalog = buildVerifiedCatalog(capabilities.xfadeTransitions);
  logger.info('ffmpeg capabilities', {
    ...
    transitions: verifiedCatalog.types.length,
    transitionsMissing: verifiedCatalog.missing,
  });
  ...
  logger.info('plan accepted', {
    ...
    fps: plan.export.fps,
    keepOriginalAudio: plan.audio.keepOriginal,
    textOverlays: plan.textOverlays.length,
  ...
  // Громкость измеряем, только если звук вообще пойдёт в ролик: при
  // keepOriginal: false он всё равно будет отброшен.
  const needsAudio = plan.audio.keepOriginal;
  ...
  for (const [i, asset] of plan.assets.entries()) {
    ...
    const probe = await analyzeAsset({
      ...
      filePath: localPath,
      asset,
      needsAudioAnalysis: needsAudio && asset.type === 'video',
  ...
  const frame = { width: plan.export.width, height: plan.export.height };
  ...
  let srtText = null;
  if (cues.length > 0) {
    srtText = toSrt(cues, plan.captions.maxLineChars);
    ...
    await writeFile(
      ...
      toAss(cues, {
        ...
        highlightColorHex: plan.captions.highlightColorHex,
        fontId: plan.captions.fontId,
        position: plan.captions.position,
        maxLineChars: plan.captions.maxLineChars,
        ...frame,
      }),
      'utf8',
    );
  }
  await reporter.report('rendering', 0.3, 'Субтитры готовы');

  // ── Текстовые слои (§4) ─────────────────────────────────────────────────
  // Текст пользователя пишется в UTF-8 .ass; в filter_complex уходит только
  // путь к файлу, поэтому спецсимволы в подписи ничего не могут сломать.
  const overlays = [];
  for (const [index, raw] of plan.textOverlays.entries()) {
    const { overlay, notes: overlayNotes, error } = normalizeOverlay(raw, {
      index,
      segments: timeline.segments,
      totalDuration: timeline.totalDuration,
    });
    if (error) {
      throw new WorkerError('PLAN_INVALID', 'Текстовый слой задан неверно.', {
        field: `plan.textOverlays[${index}]`,
        detail: error,
      });
    }
    planNotes.push(...overlayNotes);
    overlays.push(overlay);
  }

  let overlayPath = null;
  if (overlays.length > 0) {
    overlayPath = path.join(workDir, 'overlays.ass');
    await writeFile(overlayPath, overlaysToAss(overlays, frame), 'utf8');
    logger.info('text overlays prepared', {
      count: overlays.length,
      outsideSafeZone: overlays.filter((o) => !o.insideSafeZone).length,
    });
  }
  await reporter.report('rendering', 0.6, 'Текст готов');
  ...
  for (const note of [...planNotes, ...command.notes]) logger.warn('pipeline note', { note });
  ...
  const verification = await verifyOutput(env.ffprobePath, videoPath, {
    ...
    // §1 v2: ждём дорожку ровно тогда, когда сами её сделали.
  ...
  const result = {
    ...
    fps: plan.export.fps,
    videoCodec: 'h264',
    // §1 v2: без звука дорожки нет вовсе, и в результате это должно быть видно.
    audioCodec: command.hasAudio ? 'aac' : null,
    hasAudio: command.hasAudio,
    contractVersion: 2,
    planVersion: plan.planVersion,
    exportPreset: plan.export.preset,
