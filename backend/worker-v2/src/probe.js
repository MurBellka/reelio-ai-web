...
export async function verifyOutput(ffprobePath, filePath, expected) {
  ...
  // §1 v2: ролик без звука — законный результат, а не дефект. Ожидание задаёт
  // вызывающий, поэтому проверяем обе стороны: нет дорожки там, где обещали,
  // и наоборот — лишняя дорожка там, где обещали тишину.
  if (expected.hasAudio === true && !audio) {
    problems.push('в результате нет аудиопотока');
  }
  if (expected.hasAudio === false && audio) {
    problems.push('в результате есть аудиопоток, хотя звук отключён');
  }
