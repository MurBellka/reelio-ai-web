...
Обязательный минимум ответа — `assetId`, `uploadUrl`, `objectPath`; `expiresAt`
нужен клиенту, чтобы не отправлять байты по заведомо протухшей ссылке.
Остальные поля дополняют ответ и не ломают клиентов, которые их игнорируют.

> `objectPath` возвращается обязательно: клиент подставляет ровно его в
> `RenderRequest.assets[].objectPath`, а не вычисляет путь заново — иначе
> расхождение в правилах именования разошлось бы между сторонами незаметно.
