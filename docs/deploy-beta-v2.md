...
ID собирается детерминированно из типа задачи, серверного `jobId` и **номера
попытки** `enqueueSeq` (см. `buildTaskId` в `src/tasks.js`):

- **kind → префикс** из закрытого списка `TASK_KIND_PREFIXES`: `analysis.run` →
  `analysis-run`, `analysis.reap` → `analysis-reap`. **Точку** (как в
  `analysis.run`) в ID включать нельзя — именно она давала `INVALID_ARGUMENT`, и
  задача никогда не ставилась. Неизвестный kind отклоняется **до** обращения к API.
...
- **enqueueSeq → суффикс** `-a<seq>` (первая попытка `-a1`). Итог:
  `analysis-run-<jobId|d_дайджест>-a<seq>`. Полное имя — официальным
  `client.taskPath(...)`, а не конкатенацией строк.

Схема номера попытки (`enqueueSeq`, долговечно в Firestore):

- создание анализа → `enqueueSeq = 1`;
- повтор **той же** попытки (сетевой ретрай enqueue, тот же `Idempotency-Key`) →
  **то же** имя → идемпотентно (`ALREADY_EXISTS` = успех **только** для текущей
  попытки);
- явный `POST /analysis/{id}/retry` **атомарно** увеличивает `enqueueSeq`
  (`+1`) → **новое** имя.

Именно новое имя решает баг ретрая: Cloud Tasks держит имя завершённой задачи в
дедуп-окне ~1 час. При повторном использовании того же имени `createTask` вернул
бы `ALREADY_EXISTS`, код счёл бы это успехом, но новая задача **не запустилась**
бы — анализ навсегда завис бы в `queued`. Инкремент `enqueueSeq` даёт имя, не
попадающее в tombstone, поэтому retry реально ставит и запускает задачу.

Примеры: попытка 1 — `analysis-run-an_01H8XY-a1`; retry — `analysis-run-an_01H8XY-a2`.

### Watchdog зависших `queued` (§4A.9)

Долговечная защита от потерянной/недоставленной задачи — без `min-instances=1`:

- при постановке анализа планируется **отложенная Cloud Task** на
  `POST /internal/analysis/reap` (тот же OIDC: канонический audience-origin, тот
  же invoker-SA; иной target path — `INTERNAL_REAP_PATH`), c задержкой
  `ANALYSIS_QUEUE_TIMEOUT_MS` (по умолчанию 10 мин) через `scheduleTime`;
- reaper (`reapQueued`) переводит задачу в `failed` **только** если она всё ещё
  `queued`, это **та же** попытка (`enqueueSeq` совпадает) и время ожидания
  вышло. Watchdog старой попытки после retry/cancel/успеха — **no-op**;
- дополнительно — **защитная проверка**: просроченная `queued` добивается при
  обращении к задаче (`getAnalysis`) и перед резервированием слота
  (`createAnalysis` → `listActiveJobs`), даже если reap-задача не сработала;
- код ошибки — **`ANALYSIS_QUEUE_TIMEOUT`** (HTTP 503, `retryable: true`); слот
  освобождается, кредит возвращается **ровно один раз**.

IAM: `enqueuer`/`token-creator` для reap-задач те же, что и для `analysis.run`
(одна очередь, тот же invoker-SA), новых ролей не требуется.
...
- повторный запрос с тем же `Idempotency-Key` находит failed-задачу и не
  ...
  рабочую попытку;
- `#failEnqueue` меняет задачу **только при совпадении `enqueueSeq`** — сбой
  enqueue старой попытки не обрушит уже поднятую новую (конкурентный retry);
- **`ALREADY_EXISTS`** для той же попытки — это идемпотентный **успех** (задача
  уже стоит), а не 500.
